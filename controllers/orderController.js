...
exports.assignDeliveryAgent = catchAsync(async(req, res, next) => {
   const { id } = req.params; // Document _id of the order
   ...
   // Map delivery_charge from Shopify if available.
   // This assumes the order object in DB has a shipping_lines or similar we can extract from.
   // For now, let's keep it 0 or preserve existing, depending on what's available.
   // Wait, Shopify shipping lines aren't stored in our current Order model schema explicitly 
   // beyond total_price/total_tax, but we can just set it if passed or leave it 0.
   // We will just update the status and agent id for now.
